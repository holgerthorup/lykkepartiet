...
async function ftBatchFetcher() {
  const proposalExpand =
    "&$expand=Sagsstatus,Periode,Sagstype,SagAkt%C3%B8r,Sagstrin/Afstemning";
  const proposalFilter =
    "&$filter=(typeid eq 3 or typeid eq 5) and periodeid eq 151";
  const proposalUrl =
    "http://oda.ft.dk/api/Sag?$orderby=id desc" +
    proposalExpand +
    proposalFilter;
  ...
  for (const proposal of filteredProposalList) {
    const existingProposal = R.find(R.propEq("id", proposal.id))(
      existingProposalList
    );
    async function presentation() {
      const presentationUrl = `http://www.ft.dk/samling/${
        proposal.Periode.kode
      }/${proposal.Sagstype.type}/${proposal.nummerprefix +
        proposal.nummernumerisk}/${
        proposal.Periode.kode
      }_${proposal.nummerprefix +
    ...
    const results =
      stageInfo.status === "Afsluttet" && (await getVoteResults(proposal.id));
    const existingPresentation = R.path(
      ["data", "presentation"],
      existingProposal
    );
    const doNotLookForPresentation =
      existingPresentation || proposal.nummerprefix === "B"; // beslutningforslag will never get a presentation
    ...
    const upsertedProposal = {
      ...
      data: {
        ...
        presentation: doNotLookForPresentation
