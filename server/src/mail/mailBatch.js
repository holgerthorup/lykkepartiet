...
async function mailBatch(schedule) {
  ...
  for (const user of userList) {
    if (user.email_notification === schedule) {
      ...
      filteredProposalList.length &&
        emailList.push({ userId: user.id, email: user.email, filteredProposalList, firstname: user.firstname });
    }
  }
  if (!emailList.length) {
  ...
  for (const user of emailList) {
    fs.readFile(__dirname + '/newProposalsTemplate.html', 'utf8', function(err, html) {
      if (err) {
        throw err;
      }
      ...
      for (const proposal of user.filteredProposalList) {
        htmlNode('#proposal-titel-field').after(
          '<a href=https://app.lykkepartiet.dk/proposal/' +
            proposal.proposalId +
            "><strong style='line-height:24px; font-size:16px; color:#222222;'>" +
            proposal.shortTitel +
            '</strong></a><br><br><br>'
        );
      }
